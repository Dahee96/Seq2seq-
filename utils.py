...
def model_init(inp_out_dict, model, config, arch_dict, use_cuda, multi_gpu, to_do):
    ...
    for line in model:
        ...
        if operation == "compute":
            ...
            print('utils.py의 model_init부분에 있는 net', net)
            if use_cuda:
                net.cuda()
            ...
            print('utils.py에 있는 nns[arch_dict[inp1][1]]',nns[arch_dict[inp1][1]])
            print('utils.py에 있는 nns', nns)
            out_dim = net.out_dim
        ...
        if operation == "cost_nll":
            costs[out_name] = nn.CrossEntropyLoss() #nn.NLLLoss()
